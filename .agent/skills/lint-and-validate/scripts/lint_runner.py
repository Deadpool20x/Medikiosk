...
def detect_project_type(project_path: Path) -> dict:
    ...
    if package_json.exists():
        ...
        try:
            ...
            pkg_manager = "pnpm" if pnpm_lock.exists() else "npm"
            
            if "lint" in scripts:
                result["linters"].append({"name": f"{pkg_manager} lint", "cmd": [pkg_manager, "run", "lint"]})
            elif "eslint" in deps:
                result["linters"].append({"name": "eslint", "cmd": ["npx", "eslint", "."]})
            
            # Check for TypeScript / Type-check script
            if "type-check" in scripts:
                result["linters"].append({"name": f"{pkg_manager} type-check", "cmd": [pkg_manager, "run", "type-check"]})
